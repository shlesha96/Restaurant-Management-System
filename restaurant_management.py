...
def generate_bill():
    ...
    query = "insert into bill (name,quantity, rate, cost) values('{}','{}','{}','{}')".format(itemName, quantity,
                                                                                              itemRate, cost)
...
def Reportdata():
    ...
    xAxis=[]
    yAxis=[]
    cursor.execute('SELECT name, sum(distinct quantity) as totalquantity from bill')
    for row in cursor.fetchall():

        xAxis.append(row.get("name"))  # intakes the values inserted under x1, x2 and x3 to represent the x Axis
        yAxis.append(row.get("totalquantity"))  # intakes the values inserted under x1, x2 and x3 to represent the y Axis
    ...
    subplot2 = figure1.add_subplot(211)
    subplot2.bar(xAxis, yAxis, color='g')  # create the bar chart based on the input variables x1, x2, and x3

    bar2 = FigureCanvasTkAgg(figure2, root)  # create a canvas figure (matplotlib module)
    bar2.get_tk_widget().pack(side=tk.LEFT, fill=tk.BOTH, expand=0)

    figure3 = Figure(figsize=(5, 4), dpi=100)  # create a Figure (matplotlib module)
    subplot3 = figure3.add_subplot(311)
    subplot3.bar(xAxis, yAxis, color='g')  # create the bar chart based on the input variables x1, x2, and x3

    bar3 = FigureCanvasTkAgg(figure1, root)  # create a canvas figure (matplotlib module)
...
def addItem():
    ...
    conn = pymysql.connect(host="localhost", user="root", passwd="", db="billservice")
    cursor = conn.cursor()
    query = "insert into itemlist (name,nameid,rate,type,storetype) value('{}','{}','{}','{}','{}')".format(name,
                                                                                                            nameId,
                                                                                                            rate, Type,
                                                                                                            storeType)
    cursor.execute(query)
...
def updateItem():
    ...
    conn = pymysql.connect(host="localhost", user="root", passwd="", db="billservice")
    cursor = conn.cursor()
    query = "update  itemlist set name='{}',rate='{}',type='{}',storetype='{}' where nameid='{}'".format(name, rate,
                                                                                                         Type,
                                                                                                         storeType,
                                                                                                         updateItemId)
    cursor.execute(query)
...
def loginWindow():
    titleLabel = Label(window, text="RESTAURANT MANAGEMENT SYSTEM", font="Arial 10", fg="green")
...
def mainwindow():
    ...
    ReportDisplay = Button(window, text="Report", width=15, height=2, command=lambda: Reportdata())
    ReportDisplay.grid(row=1, column=3, padx=(10, 0), pady=(10, 0))
...
def itemAddWindow():
    ...
    titleLabel = Label(window, text="RESTAURANT MANAGEMENT SYSTEM", width=40, font="Arial 10", fg="green")
